...
class PointCloudProcessor:
	...
	def voxel_down_sample_and_get_indices(self, pcd, voxel_size):
		# ボクセルダウンサンプリング
		downsampled_pcd = pcd.voxel_down_sample(voxel_size)
		
		# 元の点群の座標とダウンサンプルされた点群の座標
		...
		# 各ダウンサンプルされた点に最も近い元の点を検索
		for down_point in downsampled_points:
			[success, idx, _] = kdtree.search_knn_vector_3d(down_point, 1)  # 最も近い1点を検索
			if success > 0:  # 成功した場合のみインデックスを追加
				indices.append(idx[0])
			else:
				raise ValueError(f"KD-tree search failed for point: {down_point}")

		indices = np.array(indices, dtype=np.int32).flatten()  # 1次元に整形
	...
	def process_point_cloud(self):
    	...
    	# ボクセルダウンサンプリングによる密度調整
		min_bound = np.min(numpy_measured_pcd, axis=0)
		max_bound = np.max(numpy_measured_pcd, axis=0)
		density = len_measured_pcd / np.prod(max_bound - min_bound)
		weight = 0.9 if self.target == "t" else 0.8
		...
		full_saved_mask = np.zeros(len(model_pcd.points), dtype=np.int32)

		model_pcd, indices = self.voxel_down_sample_and_get_indices(model_pcd, voxel_size)
    	...
    	# ダウンサンプリング後のSaved maskを更新
		saved_mask = saved_mask[indices]
		...
		#o3d.visualization.draw_geometries([extracted_pcd, masked_pcd, self.filter])

    	# RMSE計算
		if saved_mask.shape != predicted_mask.shape:
			rospy.logerr("Saved mask and predicted mask have different shapes")
			return
		rmse = np.sqrt(np.mean((predicted_mask - saved_mask) ** 2))
		print(f"RMSE between predicted and saved masks: {rmse:.6f}")
		# 以下、姿勢推定処理（省略可能）
		...
		est_T_cheese = result_cheese['est_T']
