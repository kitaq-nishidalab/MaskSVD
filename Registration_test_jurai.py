...
class ICP:
	...
	def __call__(self, template, source):
		...
		result_ransac = Global_optimizer.execute_global_registration(source_down, template_down, source_fpfh, template_fpfh, voxel_size)
		
		source_vis_2 = o3d.geometry.PointCloud()
		#numpy_source_vis = result_ransac.transformation.T @ np.array(source_vis.points).T
		numpy_source_vis = np.matmul(result_ransac.transformation[0:3, 0:3], np.array(source.points).T).T + result_ransac.transformation[0:3, 3]
		source_vis_2.points = o3d.utility.Vector3dVector(numpy_source_vis)
		source_vis_2.paint_uniform_color([0, 1, 0])
		###o3d.visualization.draw_geometries([template_vis, source_vis_2, o])     # テンプレ、RANSAC後ソース
...
def display_results_sample(template, source, est_T, masked_template):
  ...
  o3d.visualization.draw_geometries([template, source, transformed_source])         # テンプレ、ソース、変換後ソース、原点
