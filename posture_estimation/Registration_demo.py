...
class SVD:
	...
	def __call__(self, template, source):
		...
		##o3d.visualization.draw_geometries([source, template, o_geo])
		#############################################################################################################
		#############################################################################################################
		### 回転後ソースの主成分分析 ###
		...
		print("rotation X, Y, Z:", rotation_angle_x, rotation_angle_y, rotation_angle_z)
		print("translation :", est_T[0:3, 3])
		#print(est_T)
