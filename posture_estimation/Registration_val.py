...
class SVD:
	...
	def upsampling(self, pcd, number=1000):
		...
		mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(pcd, alpha=0.005)
	...
	def preprocess(self, template, source, pattern):
		...
		#template_pcd = self.upsampling(template_pcd, len(np.array(template_pcd.points)) * 10)
		#source_pcd = self.upsampling(source_pcd, len(np.array(source_pcd.points)) * 10)
		...
		voxel_size = 0.04  #0.02(Haris)
		template_ = template_.voxel_down_sample(voxel_size)
		source_ = source_.voxel_down_sample(voxel_size)
		### Haris3D
		#template_ = haris_img.haris3d(template_, blockSize=6, ksize=3, k=0.04, threshold_factor=0.3)
		#source_ = haris_img.haris3d(source_, blockSize=6, ksize=3, k=0.04, threshold_factor=0.3)
		#model_ = keypoint_fpfh.keypoint_fpfh(model_pcd, radius=0.55)
		### FPFH
		#template_ = keypoint_fpfh.keypoint_fpfh(template_, radius=0.7, threthold=0.1)
		#source_ = keypoint_fpfh.keypoint_fpfh(source_, radius=0.1, threthold=0.)
		### ISS
	...
	def __call__(self, template, source, pattern):
		...
		# tensorからo3d
		template, source= self.preprocess(template, source, pattern)
